...
try:
    ...
    from hdbscan import HDBSCAN     # DBSCAN with noises
...
class Loader(object):
    ...
    def load_binaries(self, directory):
        ...
        for f in os.listdir(directory):
            self.binaries[f] = Instance()   # Create an instance for one file
            self.binaries[f].load_json(directory+"/"+f, f)  # Load the files as JSON or dictionary 
            self.binaries[f].label_sample()  # Lable samples
            self.binaries[f].extract_features()  # Extract static and dynamic features
            self.binaries[f].extract_basic_features()   # Extract basic features provided by cuckoo sandbox
...
class Instance(object):
    ...
    def __init__(self):
        self.json_path = ""
        self.name = ""  # load_json
        self.report = None  # load_json
        self.total = None   # load_json
        self.positives = None   # load_json
        self.scans = None   # load_json
        self.label = None
        self.features = {}
        self.basic_features = {}
    ...
    def load_json(self, json_file, name="unknown"):
        ...
        # Load the input as a path to JSON file
        if isinstance(json_file, str):
            ...
            with open(json_file, "r") as malware_report:
                try:
                    ...
                    sys.exit(1)
        # Load the input as a dictionary object
    ...
    # ******************** QUESTION: didn't see static and dynamic fields in the report??? ********************

    # Get feasures from static binary
    # Add two features: size, FileDescription, OriginalFilename
    def feature_static_metadata(self):
        ...
        # Get type of the target file
        # Magic byte
        self.features["magic_byte"] = \
            self.report.get("target", {}).get("file", {}).get("type")

    # Get feasures from static binary
    # Add five features: signed, Comments, ProductName, LegalCopyright, InternalName, CompanyName
    ...
    # Get feasures from static binary
    # Add three features: languages, section_attrs, resource_attrs
    ...
    # Get feasures from static binary
    # Add one feature: static_imports
    ...
    # Add three features: mutex, processes, dynamic_imports
    ...
    # Get feasures from dynamic binary
    # Add one feature: files_operations
    # Which consist of: file_read, file_written, file_deleted, file_copied, file_renamed, files_opened, files_exists, files_failed
    ...
    # Get feasures from dynamic binary
    # Add four features: tcp, udp, dns, http
    ...
    # Get feasures from dynamic binary
    # Add two features: regkey_written, regkey_deleted
    ...
    # Extract features provided by cuckoo sandbox
